...
const icons: LucideIcon[] = [Sparkles, Layers, Package];
...
export const DesignCatalog = () => {
  ...
  return (
    <section id="diseno" className="bg-background px-6 md:px-10 py-14 md:py-28">
      <div className="max-w-6xl mx-auto">
        <Reveal className="text-center mb-9 md:mb-14">
          <span className="inline-block bg-tertiary text-on-tertiary rounded-full px-5 py-2 text-xs font-bold uppercase tracking-wide mb-6">
            Piezas gráficas y comunicación visual
          </span>
          <h2 className="font-display text-4xl md:text-5xl text-primary">Todo en diseño</h2>
        </Reveal>

        <RevealGroup className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {categories.map(({ icon: Icon, title, bullets, bg, fg }) => (
            <RevealItem
              key={title}
